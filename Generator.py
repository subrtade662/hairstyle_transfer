...
# taken from https://github.com/eladrich/pixel2style2pixel/blob/master/models/stylegan2/model.py
...
class EqualLinear(nn.Module):
    ...
    def forward(self, input):
        if self.activation:
            ...
            out = F.linear(input,
              (self.weight * self.scale), bias=(self.bias * self.lr_mul))
        return out
...
class ModulatedConv2d(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, kernel_size, style_dim, demodulate=True, upsample=False, downsample=False, blur_kernel=[
 1, 3, 3, 1]):
        super().__init__()
        self.eps = 1e-08
...
class StyledConv(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, kernel_size, style_dim, upsample=False, blur_kernel=[
 1, 3, 3, 1], demodulate=True):
        super().__init__()
...
class Generator(nn.Module):
    ...
    def __init__(self, size, style_dim, n_mlp, channel_multiplier=2, blur_kernel=[
 1, 3, 3, 1], lr_mlp=0.01):
        super().__init__()
        self.size = size
        ...
        for i in range(n_mlp):
            layers.append(EqualLinear(style_dim,
        ...
        self.conv1 = StyledConv((self.channels[4]),
          (self.channels[4]), 3, style_dim, blur_kernel=blur_kernel)
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.convs.append(StyledConv(in_channel,
              out_channel,
              3,
              style_dim,
              upsample=True,
              blur_kernel=blur_kernel))
            self.convs.append(StyledConv(out_channel,
              out_channel, 3, style_dim, blur_kernel=blur_kernel))
    ...
    def mean_latent(self, n_latent):
        latent_in = torch.randn(n_latent,
          (self.style_dim), device=(self.input.input.device))
...
class ConvLayer(nn.Sequential):
    ...
    def __init__(self, in_channel, out_channel, kernel_size, downsample=False, blur_kernel=[
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, blur_kernel=[
 1, 3, 3, 1]):
        super().__init__()
        self.conv1 = ConvLayer(in_channel, in_channel, 3)
        self.conv2 = ConvLayer(in_channel, out_channel, 3, downsample=True)
        self.skip = ConvLayer(in_channel,
          out_channel, 1, downsample=True, activate=False, bias=False)
